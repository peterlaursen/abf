...
HeaderSize += fwrite(&TitleLength, 1, sizeof(short), fin);
const char* _Title = Title.c_str();
HeaderSize += fwrite(_Title, 1, TitleLength, fin);
unsigned short AuthorLength = Author.length();
HeaderSize += fwrite(&AuthorLength, 1, sizeof(short), fin);
const char* _Author = Author.c_str();
HeaderSize += fwrite(_Author, 1, AuthorLength, fin);
unsigned short TimeLength = Time.length();
HeaderSize += fwrite(&TimeLength, 1, sizeof(short), fin);
const char* _Time = Time.c_str();
HeaderSize += fwrite(_Time, 1, TimeLength, fin);
...
void AbfDecoder::ReadHeader() {
	...
	printf("Version %d.%d", Major, Minor);

	unsigned short TitleLength;
	fread(&TitleLength, sizeof(short), 1, fin);
	printf("Title is %d bytes\n", TitleLength);
	Title = new char[TitleLength]+1;
	fread(Title, 1, TitleLength, fin);
Title[TitleLength] = '\0';
printf("The title is %s\n", Title);
unsigned short AuthorLength;
fread(&AuthorLength, sizeof(short), 1, fin);
printf("Length of author string: %d\n", AuthorLength);	
Author = new char[AuthorLength]+1;
fread(Author, 1, AuthorLength, fin);
